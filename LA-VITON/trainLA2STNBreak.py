...
def train_gmm(opt, train_loader, model, board):
    ...
    for step in range(opt.keep_step + opt.decay_step):
        ...
        warped_grid = F.grid_sample(warped_gridaffine, gridtps, padding_mode='zeros')
        ...
        if (step+1) % opt.display_count == 0:
            ...
            print('step: %8d, time: %.3f, loss: %4f, 0.5*Lpersp: %.6f, 0.5*Lwarp: %.6f' % (step+1, t, loss.item(), (0.5*Lpersp).item(), (0.5*Lwarp).item()), flush=True)
