...
class FeatureRegressionAffine(nn.Module):
    def __init__(self, output_dim=6, use_cuda=True, batch_normalization=True, kernel_sizes=[7, 5, 5],
                 channels=[225, 128, 64]):
...
class AffineGridGen(nn.Module):
    ...
    def forward(self, theta):  # theta shape N x 2 x 3 for 2D and N x 3 x 4 for 3D
...
class LAGMM(nn.Module):
    def __init__(self, opt):
        super(LAGMM, self).__init__()
        self.extractionA = FeatureExtraction(22, ngf=64, n_layers=3, norm_layer=nn.BatchNorm2d)
        self.extractionB = FeatureExtraction(3, ngf=64, n_layers=3, norm_layer=nn.BatchNorm2d)
        self.l2norm = FeatureL2Norm()
        self.correlation = FeatureCorrelation()
        self.regression1 = FeatureRegression(input_nc=192, output_dim=2 * 3, use_cuda=True)
        self.regression2 = FeatureRegression(input_nc=192, output_dim=2 * opt.grid_size ** 2, use_cuda=True)
        self.gridGen = TpsGridGen(opt.fine_height, opt.fine_width, use_cuda=True, grid_size=opt.grid_size)
    ...
    def forward(self, inputA, inputB):
        ...
        featureB = self.l2norm(featureB)
        correlation = self.correlation(featureA, featureB)
        thetaaffine = self.regression1(correlation).reshape(featureA.shape[0], 2, 3)
        # print("theta shape:",theta.shape)
        gridaffine = self.affineGen(thetaaffine)
        Ipersp = F.grid_sample(inputB, gridaffine, padding_mode='zeros')

        # second branch for TPS
        featureA2 = self.extractionA(inputA)
        featureB2 = self.extractionB(Ipersp)
        featureA2 = self.l2norm(featureA2)
        featureB2 = self.l2norm(featureB2)
        correlation2 = self.correlation(featureA2, featureB2)
