...
class CPDataset(data.Dataset):
    ...
    def __init__(self, opt):
        ...
        self.c_names = c_names  # without changing from test pairs
    ...
    def __getitem__(self, index):
        ...
        # c_name = im_name.split("_")[0] + "_1.jpg" #same cloth test
        # cloth image & cloth mask
        if self.stage == 'GMM':
            c = Image.open(osp.join(self.data_path, 'cloth', c_name))  #same cloth test
            cm = Image.open(osp.join(self.data_path, 'cloth-mask', c_name))  #same cloth test
            # c = Image.open(osp.join(self.data_path, 'cloth-cut', c_name))  #same cloth test
            # cm = Image.open(osp.join(self.data_path, 'cloth-mask-cut', c_name)).convert('L')  #same cloth test
        else:
            c = Image.open(osp.join(self.data_path, 'warp-cloth', im_name))  # full test diff
            cm = Image.open(osp.join(self.data_path, 'warp-mask', im_name))  # full test diff
            # c = Image.open(osp.join(self.data_path, 'warp-cloth-full-diff', c_name))  # full test diff
            # cm = Image.open(osp.join(self.data_path, 'warp-mask-full-diff', c_name))  # full test diff
            # c = Image.open(osp.join(self.data_path, 'warp-cloth-full-same', im_name))  # full test same
            # cm = Image.open(osp.join(self.data_path, 'warp-mask-full-same', im_name))  # full test same
            # c = Image.open(osp.join(self.data_path, 'warp-cloth', im_name))  # full test same
            # cm = Image.open(osp.join(self.data_path, 'warp-mask', im_name))  # full test same
            # c = Image.open(osp.join(self.data_path, 'warp-cloth', im_name))  # for new test pairs
            # cm = Image.open(osp.join(self.data_path, 'warp-mask', im_name))  # for new test pairs
        ...
        im_parse = Image.open(
            ...
            # osp.join(self.data_path, 'image-parse-new', parse_name)) #update new
        ...
        # im_hand = im * phand - (1 - phand)  # [-1,1], fill 0 for other parts
        ...
        result = {
            ...
            # 'hand': im_hand,           # for visualization
