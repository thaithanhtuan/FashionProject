...
class FeatureCorrelation(nn.Module):
    ...
    def forward(self, feature_A, feature_B):
        ...
        #print("feature_A.shape:", feature_A.shape)# [4x512x192]
        feature_B = feature_B.view(b,c,h*w).transpose(1,2)
        #print("feature B.shpae:", feature_B.shape)#[4x192x512]
        # perform matrix mult.
        feature_mul = torch.bmm(feature_B,feature_A)
        #print("feature_mul.shape", feature_mul.shape)#[4x192x192]
        correlation_tensor = feature_mul.view(b,h,w,h*w).transpose(2,3).transpose(1,2)
        #print("correlation_tensor.shape", correlation_tensor.shape)#[4x192x16x12]
...
class TpsGridGen(nn.Module):
    ...
    def forward(self, theta):
        #grid_X = array([[-1., ... , 1.],[-1., ... , 1.],...,[-1., ... , 1.]])
        #grid_Y = array([[-1.,....,-1.],[..],...,[1.,...,1.]])
        #print("theta.shape", theta.shape) #[4x50]
        #print("grid_X.shape", self.grid_X.shape)#[1,256,192,1]
        #print("grid_Y.shape", self.grid_Y.shape)#[1,256,192,1]
        warped_grid = self.apply_transformation(theta,torch.cat((self.grid_X,self.grid_Y),3))
        #print("warped_grid.shape", warped_grid.shape)#[4,256,192,2]
    ...
    def apply_transformation(self,theta,points):#theta [batchsize, (gridsize ^2 *2)] (4,50)
        if theta.dim()==2:
            theta = theta.unsqueeze(2).unsqueeze(3)
            #print("theta.shape", theta.shape) #[4,50,1,1]
