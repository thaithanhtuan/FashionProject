...
def main(argv=None):
    ...
    for i in range(len(pred_eUnet)):
        ...
        if (pred_eUnet[i] >= pred_eFCN[i]):
            sum_better_error_unet = sum_better_error_unet + (pred_eUnet[i] - pred_eFCN[i])
        else:
            sum_better_error_FCN = sum_better_error_FCN - (pred_eUnet[i] - pred_eFCN[i])
    ...
    print("sum_better_error_unet/num_unet_good:", sum_better_error_unet/float(num_unet_good))
    print("sum_better_error_FCN/num_FCN_good:", sum_better_error_FCN/float(num_FCN_good))
